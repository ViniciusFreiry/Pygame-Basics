...
# Cria o objeto retângulo do botão (hitbox)
...
while rodando:
    for evento in pygame.event.get():
        if evento.type == pygame.QUIT:
            ...
            if botao.collidepoint(evento.pos):  # Verificação de colisão com o botão
                rodando = False

    # Captura os eventos de tecla
    teclas = pygame.key.get_pressed()

    # Controle de movimento para a direita
    if teclas[pygame.K_RIGHT] and pos_x < largura - quadro_largura:
        if not (botao.colliderect(pygame.Rect(pos_x + movimento, pos_y, quadro_largura, quadro_altura))): 
    ...
    if teclas[pygame.K_DOWN] and pos_y < altura - quadro_altura:
        if not (botao.colliderect(pygame.Rect(pos_x, pos_y + movimento, quadro_largura, quadro_altura))):
